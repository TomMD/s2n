...
static int s2n_config_init(struct s2n_config *config)
{
    ...
    notnull_check(config->cert_and_key_pair_map = s2n_map_new());
    GUARD(s2n_map_complete(config->cert_and_key_pair_map));
...
static int s2n_config_cleanup(struct s2n_config *config)
{
    ...
    GUARD(s2n_map_free(config->cert_and_key_pair_map));

    return 0;
}

static int s2n_config_update_cert_and_key_pair_map(struct s2n_map *cert_and_key_pair_map,
        ...
        struct s2n_cert_chain_and_key *cert_key_pair)
{
    ...
    if (s2n_map_lookup(cert_and_key_pair_map, name, &map_value) == 0) {
        struct s2n_cert_chain_and_key_map_value value = {{ 0 }};
        value.certs[auth_method] = cert_key_pair;
        map_value.data = (uint8_t *) &value;
        map_value.size = sizeof(struct s2n_cert_chain_and_key_map_value);

        GUARD(s2n_map_unlock(cert_and_key_pair_map));
        GUARD(s2n_map_add(cert_and_key_pair_map, name, &map_value));
        GUARD(s2n_map_complete(cert_and_key_pair_map));
    } else {
        struct s2n_cert_chain_and_key_map_value *value = (void *) map_value.data;;
...
static int s2n_config_build_cert_and_key_pair_map(struct s2n_map *cert_and_key_pair_map, struct s2n_cert_chain_and_key *cert_key_pair)
{
    if (s2n_array_num_elements(cert_key_pair->san_names) == 0) {
        for (int i = 0; i < s2n_array_num_elements(cert_key_pair->cn_names); i++) {
            ...
            GUARD(s2n_config_update_cert_and_key_pair_map(cert_and_key_pair_map, cn_name, cert_key_pair));
        }
    } else {
        for (int i = 0; i < s2n_array_num_elements(cert_key_pair->san_names); i++) {
            ...
            GUARD(s2n_config_update_cert_and_key_pair_map(cert_and_key_pair_map, san_name, cert_key_pair));
        }
    }
...
int s2n_config_add_cert_chain_and_key_to_store(struct s2n_config *config, struct s2n_cert_chain_and_key *cert_key_pair)
{
    ...
    notnull_check(config->cert_and_key_pair_map);
    ...
    *to_insert = cert_key_pair;
    GUARD(s2n_config_build_cert_and_key_pair_map(config->cert_and_key_pair_map, cert_key_pair));
