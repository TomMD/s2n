...
/* This is the current memory usage per connection, we should be really
 * careful going beyong this value. */
#define MAX_MEM_PER_CONNECTION (136 * 1024)

#define MAX_ALLOWED_MEM_DIFF ((size_t) 2 * CONNECTIONS * MAX_MEM_PER_CONNECTION)


size_t get_vm_data_size()
{
#ifdef __linux__
    long page_size;
    ...
    if (fscanf(status_file, "%zu %zu %zu %zu %zu %zu %zu", &size, &resident, &share, &text, &lib, &data, &dt) < 7) {
        fclose(status_file);
        return (size_t) -1;
    }
    fclose(status_file);
...
int main(int argc, char **argv)
{
    ...
    size_t vm_data_initial = get_vm_data_size();
    EXPECT_NOT_EQUAL(vm_data_initial, (size_t) -1);
    ...
    size_t vm_data_after_allocation = get_vm_data_size();
    EXPECT_NOT_EQUAL(vm_data_after_allocation, (size_t) -1);

    for (int i = 0; i < CONNECTIONS; i++) {
        EXPECT_SUCCESS(s2n_negotiate_test_server_and_client(servers[i], clients[i]));
    }

    size_t vm_data_after_handshakes = get_vm_data_size();
    EXPECT_NOT_EQUAL(vm_data_after_handshakes, (size_t) -1);
