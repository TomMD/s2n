...
static int s2n_find_cert_matches(struct s2n_map *cert_and_key_pair_map,
        ...
        uint8_t *match_exists)
{
    ...
    if (s2n_map_lookup(cert_and_key_pair_map, dns_name, &map_value) == 1) {
...
int s2n_conn_find_name_matching_certs(struct s2n_connection *conn)
{
    ...
    GUARD(s2n_find_cert_matches(conn->config->cert_and_key_pair_map,
                &normalized_name,
