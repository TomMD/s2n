...
S2N_BLOB_LABEL(s2n_tls13_label_derived, "derived")

S2N_BLOB_LABEL(s2n_tls13_label_client_early_traffic, "c e traffic")
S2N_BLOB_LABEL(s2n_tls13_label_early_exporter_master_secret, "e exp master")

S2N_BLOB_LABEL(s2n_tls13_label_client_handshake_traffic, "c hs traffic")
S2N_BLOB_LABEL(s2n_tls13_label_server_handshake_traffic, "s hs traffic")

S2N_BLOB_LABEL(s2n_tls13_label_client_application_traffic, "c ap traffic")
S2N_BLOB_LABEL(s2n_tls13_label_server_application_traffic, "s ap traffic")
...
S2N_BLOB_LABEL(s2n_tls13_label_key, "key")
S2N_BLOB_LABEL(s2n_tls13_label_iv, "iv")

static const struct s2n_blob zero_length_blob = { .data = NULL, .size = 0 };

/* Message transcript hash based on selected HMAC algorithm */
static int s2n_tls13_message_hash(struct s2n_tls13_keys *keys, const struct s2n_blob *message, struct s2n_blob *message_digest)
{
...
int s2n_tls13_derive_early_secrets(struct s2n_tls13_keys *keys)
{
    ...
    GUARD(s2n_tls13_message_hash(keys, &zero_length_blob, &message_digest));
    GUARD(s2n_hkdf_expand_label(&keys->hmac, keys->hmac_algorithm, &keys->current_secret, &s2n_tls13_label_derived, &message_digest, &keys->derive_secret));
                                        ...
                                        struct s2n_blob *server_secret)
{
    ...
    GUARD(s2n_hkdf_expand_label(&keys->hmac, keys->hmac_algorithm, &keys->current_secret, &s2n_tls13_label_client_handshake_traffic, &message_digest, client_secret));
    GUARD(s2n_hkdf_expand_label(&keys->hmac, keys->hmac_algorithm, &keys->current_secret, &s2n_tls13_label_server_handshake_traffic, &message_digest, server_secret));
...
int s2n_tls13_derive_application_secrets(struct s2n_tls13_keys *keys, struct s2n_hash_state *hashes, struct s2n_blob *client_secret, struct s2n_blob *server_secret)
{
    ...
    /* produce client + server traffic secrets */
    GUARD(s2n_hkdf_expand_label(&keys->hmac, keys->hmac_algorithm, &keys->current_secret, &s2n_tls13_label_client_application_traffic, &message_digest, client_secret));
    GUARD(s2n_hkdf_expand_label(&keys->hmac, keys->hmac_algorithm, &keys->current_secret, &s2n_tls13_label_server_application_traffic, &message_digest, server_secret));
...
{
    GUARD(s2n_hkdf_expand_label(&keys->hmac, keys->hmac_algorithm, secret, &s2n_tls13_label_key, &zero_length_blob, key));
    GUARD(s2n_hkdf_expand_label(&keys->hmac, keys->hmac_algorithm, secret, &s2n_tls13_label_iv, &zero_length_blob, iv));
    return 0;
}
